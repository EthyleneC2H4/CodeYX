...
def _session_path(codeyx_dir: Path) -> Path:
    return codeyx_dir / SESSION_FILENAME
...
def load_worktree_session(codeyx_dir: Path) -> WorktreeSession | None:
    path = _session_path(codeyx_dir)
    if not path.exists():
        return None
    try:
