...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".codeyx", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".codeyx" / "history"
...
class CodeYXApp(App):
    ...
    def on_mount(self) -> None:
        self.register_theme(_CODEX_THEME)
        self.theme = "codeyx"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".codeyx" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".codeyx" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".codeyx" / "permissions.local.yaml",
            ),
        ...
        from codeyx.config import WorktreeConfig
        ...
        from codeyx.tools.team_create import TeamCreateTool
        from codeyx.tools.team_delete import TeamDeleteTool
        ...
        from codeyx.commands.handlers.trace import create_trace_command
        ...
        from codeyx.tools.synthetic_output import SyntheticOutputTool
