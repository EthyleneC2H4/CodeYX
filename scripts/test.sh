...
start() {
    ...
    while [ $elapsed -lt $timeout ]; do
        if tmux capture-pane -t "$SESSION" -p 2>/dev/null | grep -q "Send a message"; then
            echo "CodeYX started successfully"
            return 0
        fi
        sleep 1
        elapsed=$((elapsed + 1))
    done
    echo "ERROR: CodeYX did not start within ${timeout}s"
    capture
    return 1
}
...
stop() {
    ...
    sleep 1
    tmux kill-session -t "$SESSION" 2>/dev/null || true
    echo "CodeYX session stopped"
}

smoke() {
    echo "=== CodeYX Smoke Test ==="

    echo "[1/4] Starting CodeYX..."
    start
...
case "${1:-help}" in
    ...
    *)
        ...
        echo ""
        echo "Commands:"
        echo "  start              Start CodeYX in tmux"
