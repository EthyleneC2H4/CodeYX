...
- CI 修复：工作流因仓库长期无活动被 GitHub 自动禁用（`disabled_inactivity`），Actions 页展示的失败全部是旧提交 `ef375eb` 的每周定时运行；重构推送后工作流已随对 test.yml 的修改重新启用，lint 门禁从 E9/F63/F7 关键子集升级为全量 ruff（代码库已全绿）。Run #20/#21 双验证全绿后，应用户要求移除 `schedule` 周定时触发器——CI 现仅在 push / PR / 手动 dispatch 时运行，integration job 保留给手动 dispatch。
